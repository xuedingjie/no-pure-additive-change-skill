...
| Add a business value field | Add field and display logic only | No field semantics, valid-record filters, idempotency, rollback, or verification path |
| Add a state/status | Add enum and if-else branch | No clear ownership, valid values, transition rules, or compatibility strategy |
...
| Propose migration | Describe backfill only | No mapping, consistency verification, idempotency, compatibility, rollback tests |
...
| Only table/field existence is inspected | Treat all persisted rows as business truth | Must confirm effective-record filters, field/status semantics, invalid/deleted/failed/test/migrated exclusion, and verification path |
...
- State effective-record filters and exclude invalid, deleted, failed, test, historical, temporary, and migrated data when they should not participate.
- State field semantics, status semantics, and verification path when relevant.
